...
def generate_script():
    ...
    try:
        ...
        prompt = f"""Generate a Python Playwright pytest script based on a recorded JS file. Follow these rules precisely.

**CRITICAL DIRECTIVE: AVOID HALLUCINATION**
Your primary goal is to faithfully translate the selectors from the provided JS file to prevent errors. Do not invent or "hallucinate" selectors, text, or labels that are not explicitly present in the JS file's code or its accompanying comments.
...
**SELECTOR GENERATION HIERARCHY (Follow these rules IN ORDER for every step):**
1.  **Direct Translation (Default Rule):** Your default action is to use the exact selector from the JS file within `page.locator()`.
    -   *Example:* If JS has `page.click('.ant-checkbox-input')`, the Python code MUST be `page.locator('.ant-checkbox-input').click()`.

2.  **Test ID Conversion:** If the JS selector is a `data-testid` (e.g., `[data-testid="company-email-input"]`), you MUST convert it to the `page.get_by_test_id()` method.
    -   *Example:* `page.get_by_test_id("company-email-input").click()`

3.  **Role/Text Conversion (ONLY with explicit evidence):** You may ONLY convert a selector to `page.get_by_role()` or `page.get_by_text()` if the JS file's **comments** for that specific line explicitly state the visible text.
    -   *Example:* If JS is `page.click('button')` and its comment is `// Note: The button's visible text is "Sign In"`, then you MUST generate `page.get_by_role('button', name='Sign In').click()`.
    -   *If no such comment exists, you MUST fall back to Rule #1: `page.locator('button').click()`.*

**RELIABILITY RULES:**
-   **No Assumptions:** Do not assume labels or roles. If the JS file provides a selector like `.ant-checkbox-input` and there are no comments about its visible text, you MUST use `page.locator('.ant-checkbox-input')`. Do not guess a label like "I agree...".
-   **Warnings:** If you are forced to use a generic CSS selector (like `.ant-checkbox-input` or `button:nth-child(3)`) because no better option is available according to the rules above, you MUST add a `# WARNING: The original selector is generic and may be unreliable.` comment before that line.
-   **Waits:**
    -   After every action that causes navigation (`page.goto` or a click that loads a new page), you MUST add `page.wait_for_load_state('networkidle')`.
    -   Before every action on an element (like `.click()` or `.fill()`), you MUST first wait for it to be visible with an `expect` call on the chosen locator. *Example: `expect(page.locator('.my-button')).to_be_visible()`*.
...
Generate tests for these specific test cases:
{selected_tests}
...
def generate_full_flow_script():
    ...
    try:
        ...
        website_url = data.get("website_url", "")
        logger.info(
            "Processing full flow script generation request",
            website_url=website_url,
            js_content_length=len(js_file_content),
        )
        # This prompt is specifically for generating a single, end-to-end test script.
        prompt = f"""Generate a Python Playwright pytest script that covers the complete flow from the recorded JS file. Follow these rules precisely.
...
Generate a complete flow test named `test_complete_end_to_end_workflow`. This should be a single test function that follows the exact sequence of actions recorded in the JS file.

Output ONLY the raw Python code.
"""
        logger.info("Calling LLM for full flow script generation")
        llm_start_time = time.time()
        response = llm.invoke(prompt)
        llm_duration = time.time() - llm_start_time
        logger.info(
            "LLM response received for full flow script generation",
            response_length=len(response.content),
            llm_duration=llm_duration,
        )
        script = clean_llm_output(response.content)
        logger.debug(
            "Full flow script generated",
            script_length=len(script),
        ...
        if missing_requirements:
            logger.error(
                "Generated full flow script missing required components",
                missing_requirements=missing_requirements,
                script_preview=script[:500],
            )
            raise ValueError(
        ...
        logger.info(
            "Full flow script validation passed",
            script_length=len(script),
            has_fixtures=True,
            has_imports=True,
        )
        ...
        logger.error(
            "Error in generate_full_flow_script",
            error=str(e),
            error_type=type(e).__name__,
        )
        return jsonify({"error": str(e)}), 500
