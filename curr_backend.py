...
5. **Verbatim-Flow / Sanity Test (Single):**  
   - When the user says *"convert JS to pytest"*, *"run recorded JS flow"*, *"sanity"*, etc., create **exactly one** test function that reproduces the recorded JS actions **verbatim** (step-by-step, using only the actions and selectors from the JS file). 
   - **IMPORTANT**: Add proper waits and error handling:
     - Before each action, wait for the element to be visible: `page.wait_for_selector("selector", timeout=10000)`
     - After navigation actions, wait for page load: `page.wait_for_load_state("networkidle")`
     - Use try-catch blocks for critical actions that might fail
     - Add explicit waits between actions if needed: `page.wait_for_timeout(2000)`
   - **Final Assertion**: Instead of checking for a specific element, use a more robust final assertion:
     - Check that the page has loaded successfully: `expect(page).to_have_url(re.compile(r".*"))` (any URL)
     - OR check that the page title is not empty: `expect(page).to_have_title(re.compile(r".+"))`
     - OR check that the page has loaded: `page.wait_for_load_state("networkidle")`
     - Do NOT use specific element selectors for the final assertion unless they are guaranteed to be present
   - Only add a single assertion at the end of the test. Do not invent selectors or error messages.
        ...
        for test in report.get("tests", []):
            ...
            if outcome != "passed":
                ...
                if isinstance(longrepr, dict):
                    # Try multiple nested fields for failure details
                    reason = (
                        longrepr.get("reprcrash", {}).get("message")
                        or longrepr.get("message")
                        or longrepr.get("reprtraceback", {})
                        .get("entries", [{}])[-1]
                        .get("line")
                        or longrepr.get("reprtraceback", {})
                        .get("reprcrash", {})
                        .get("message")
                        or json.dumps(longrepr)[:500]  # trim if massive
                    )
                elif isinstance(longrepr, str) and longrepr.strip():
                    # If it's a string, use it directly
                    reason = longrepr[:500]  # trim if too long
                else:
                    # Fallback: try to extract from stderr if available
                    if result.stderr:
                        # Look for the specific test failure in stderr
                        test_name = nodeid.split("::")[-1] if "::" in nodeid else nodeid
                        stderr_lines = result.stderr.split("\n")
                        for i, line in enumerate(stderr_lines):
                            if test_name in line and "FAILED" in line:
                                # Get the next few lines as context
                                context_lines = stderr_lines[i : i + 5]
                                reason = " | ".join(
                                    [l.strip() for l in context_lines if l.strip()]
                                )
                                break
                        else:
                            reason = "Test failed - check logs for details"
                    else:
                        reason = "Test failed - no detailed error information available"

                # Add additional debugging info from stdout if available
                if result.stdout:
