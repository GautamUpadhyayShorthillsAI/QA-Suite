...
def run_script():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                # Run pytest with enhanced reporting and detailed output
                cmd = [
                    "pytest",
                    script_path,
                    "--json-report",
                ...
                for test in report.get("tests", []):
                    ...
                    if outcome == "passed":
                        ...
                        # Extract error details from test call
                        call_info = test.get("call", {})
                        if call_info:
                            # Get error message and traceback
                            if "longrepr" in call_info:
                                failure_details["error_message"] = call_info["longrepr"]

                            # Get stdout/stderr if available
                            if "stdout" in call_info:
                                failure_details["stdout"] = call_info["stdout"]
                            if "stderr" in call_info:
                                failure_details["stderr"] = call_info["stderr"]

                            # Get traceback
                            if "traceback" in call_info:
                                failure_details["traceback"] = call_info["traceback"]
                    ...
                    logs.append(
                        {
                            ...
                            "details": failure_details if outcome != "passed" else None,
                        }
                    )
                        ...
                        logs.append(
                            {
                                ...
                                "action": call["name"],
                                "result": (
                                    "Pass" if call["status"] == "passed" else "Fail"
                                ),
